...
async function saveChannel(channel: string, config: Record<string, any>, loading: { value: boolean }) {
	...
	try {
		...
		Message.success('Epay（易支付）配置已保存')
	} catch {
		// interceptor handles error
	} finally {
		loading.value = false
	...
	<div>
		...
		<div class="space-y-4">
			...
			<ACard :bordered="false" :loading="settingsLoading" title="全局设置">
				...
				<template #actions>
					<AButton type="primary" :loading="settingsSaving" @click="handleSettingsSave">保存设置</AButton>
				</template>
			</ACard>
			...
			<ACard :bordered="false" :loading="channelsLoading">
				...
				<template #actions>
					<AButton type="primary" :loading="epaySaving" @click="handleEpaySave">保存渠道配置</AButton>
				</template>
			</ACard>
		</div>
	</div>
