import type { RouteRecordRaw } from 'vue-router'

const adminRoutes: RouteRecordRaw[] = [
  {
    path: '/admin/setup',
    name: 'AdminSetup',
    component: () => import('@/views/SetupPage.vue'),
    meta: { requiresAuth: false, title: '系统初始化' },
  },
  {
    path: '/admin/login',
    name: 'AdminLogin',
    component: () => import('@/views/LoginPage.vue'),
    meta: { requiresAuth: false, title: '管理员登录' },
  },
  {
    path: '/admin',
    component: () => import('@/views/AdminLayout.vue'),
    meta: { requiresAuth: true },
    children: [
      {
        path: '',
        name: 'AdminDashboard',
        component: () => import('@/views/DashboardPage.vue'),
        meta: { title: '仪表盘', perm: 'dashboard:view' },
      },
      {
        path: 'workbench',
        name: 'AdminWorkbench',
        component: () => import('@/views/WorkbenchPage.vue'),
        meta: { title: '工作台', perm: 'dashboard:view' },
      },
      {
        // 权限不足的落地页：路由守卫拦截越权访问后跳到这里。
        // 接口层本就会 403，但直接给一个空白页加一片报错的体验很差。
        path: '403',
        name: 'AdminForbidden',
        component: () => import('@/views/ForbiddenPage.vue'),
        meta: { title: '无权访问' },
      },
      {
        path: 'profile',
        name: 'AdminProfile',
        component: () => import('@/views/system/ProfilePage.vue'),
        meta: { title: '个人信息' },
      },
      {
        path: 'users',
        name: 'AdminUsers',
        component: () => import('@/views/system/UsersPage.vue'),
        meta: { title: '用户管理', perm: 'user:view' },
      },
      {
        path: 'plugins',
        name: 'AdminPlugins',
        component: () => import('@/views/system/PluginsPage.vue'),
        meta: { title: '插件管理', perm: 'system:plugin' },
      },
      {
        path: 'models',
        name: 'AdminModels',
        component: () => import('@/views/ai/ModelsPage.vue'),
        meta: { title: '模型管理', perm: 'model:view' },
      },
      {
        path: 'model-groups',
        name: 'AdminModelGroups',
        component: () => import('@/views/ai/ModelGroupsPage.vue'),
        meta: { title: '模型分组', perm: 'model:view' },
      },
      {
        path: 'channels',
        name: 'AdminChannels',
        component: () => import('@/views/ai/ChannelsPage.vue'),
        meta: { title: '渠道管理', perm: 'channel:view' },
      },
      {
        path: 'channels/:id',
        name: 'AdminChannelDetail',
        component: () => import('@/views/ai/ChannelDetailPage.vue'),
        meta: { title: '渠道详情', perm: 'channel:view' },
      },
      {
        path: 'usage-logs',
        name: 'AdminUsageLogs',
        component: () => import('@/views/tenant/UsageLogsPage.vue'),
        meta: { title: '用量日志', perm: 'billing:view' },
      },
      {
        path: 'transactions',
        name: 'AdminTransactions',
        component: () => import('@/views/finance/TransactionsPage.vue'),
        meta: { title: '交易流水', perm: 'billing:view' },
      },
      {
        path: 'orders',
        name: 'AdminOrders',
        component: () => import('@/views/finance/OrdersPage.vue'),
        meta: { title: '订单管理', perm: 'order:view' },
      },
      {
        // 旧"支付设置"独立页已并入系统设置（payment 分类），URL 保留重定向兼容
        path: 'payment-settings',
        redirect: { name: 'AdminSettings' },
      },
      {
        path: 'redemptions',
        name: 'AdminRedemptions',
        component: () => import('@/views/finance/RedemptionsPage.vue'),
        meta: { title: '兑换码管理', perm: 'redemption:view' },
      },
      {
        path: 'promo-codes',
        name: 'AdminPromoCodes',
        component: () => import('@/views/finance/PromoCodesPage.vue'),
        meta: { title: '优惠码管理', perm: 'promo:view' },
      },
      {
        path: 'sessions',
        name: 'AdminSessions',
        component: () => import('@/views/security/SessionsPage.vue'),
        meta: { title: '会话管理' },
      },
      {
        path: 'permissions',
        name: 'AdminPermissions',
        component: () => import('@/views/security/PermissionsPage.vue'),
        meta: { title: '角色权限', perm: 'user:view' },
      },
      {
        path: 'login-history',
        name: 'AdminLoginHistory',
        component: () => import('@/views/security/LoginHistoryPage.vue'),
        meta: { title: '登录历史', perm: 'audit:view' },
      },
      {
        path: 'tenant-login-history',
        name: 'AdminTenantLoginHistory',
        component: () => import('@/views/security/TenantLoginHistoryPage.vue'),
        meta: { title: '租户登录历史', perm: 'audit:view' },
      },
      {
        path: 'task-logs',
        name: 'AdminTaskLogs',
        component: () => import('@/views/ai/TaskLogsPage.vue'),
        meta: { title: '任务日志', perm: 'task:view' },
      },
      {
        path: 'tenants',
        name: 'AdminTenants',
        component: () => import('@/views/tenant/TenantsPage.vue'),
        meta: { title: '租户列表', perm: 'tenant:view' },
      },
      {
        path: 'tenants/:id',
        name: 'AdminTenantDetail',
        component: () => import('@/views/tenant/TenantDetailPage.vue'),
        meta: { title: '租户详情', perm: 'tenant:view' },
      },
      {
        path: 'tenant-levels',
        name: 'AdminTenantLevels',
        component: () => import('@/views/tenant/TenantLevelsPage.vue'),
        meta: { title: '租户级别', perm: 'tenant:view' },
      },
      {
        path: 'members',
        name: 'AdminMembers',
        component: () => import('@/views/tenant/MembersPage.vue'),
        meta: { title: '成员列表', perm: 'member:view' },
      },
      {
        path: 'audit',
        name: 'AdminAudit',
        component: () => import('@/views/security/AuditPage.vue'),
        meta: { title: '操作日志', perm: 'audit:view' },
      },
      {
        path: 'request-audit-logs',
        name: 'AdminRequestAuditLogs',
        component: () => import('@/views/security/RequestAuditLogsPage.vue'),
        meta: { title: '请求审计日志', perm: 'audit:view' },
      },
      {
        path: 'notification-templates',
        name: 'AdminNotificationTemplates',
        component: () => import('@/views/operation/NotificationTemplatesPage.vue'),
        meta: { title: '通知模板', perm: 'operation:view' },
      },
      {
        path: 'messages',
        name: 'AdminMessages',
        component: () => import('@/views/operation/MessagesPage.vue'),
        meta: { title: '消息管理', perm: 'operation:view' },
      },
      {
        path: 'email-logs',
        name: 'AdminEmailLogs',
        component: () => import('@/views/operation/EmailLogsPage.vue'),
        meta: { title: '邮件发送记录', perm: 'operation:view' },
      },
      {
        path: 'announcements',
        name: 'AdminAnnouncements',
        component: () => import('@/views/operation/AnnouncementsPage.vue'),
        meta: { title: '公告管理', perm: 'operation:view' },
      },
      {
        path: 'tickets',
        name: 'AdminTickets',
        component: () => import('@/views/operation/TicketsPage.vue'),
        meta: { title: '工单管理', perm: 'support:view' },
      },
      {
        path: 'feedback',
        name: 'AdminFeedback',
        component: () => import('@/views/operation/FeedbackPage.vue'),
        meta: { title: '反馈管理', perm: 'support:view' },
      },
      {
        path: 'settings',
        name: 'AdminSettings',
        component: () => import('@/views/system/SettingsPage.vue'),
        meta: { title: '系统设置', perm: 'system:view' },
      },
      {
        path: 'files',
        name: 'AdminFiles',
        component: () => import('@/views/system/FilesPage.vue'),
        meta: { title: '文件管理', perm: 'file:view' },
      },
      {
        path: 'help-categories',
        name: 'AdminHelpCategories',
        component: () => import('@/views/system/HelpCategoriesPage.vue'),
        meta: { title: '帮助分类', perm: 'support:view' },
      },
      {
        path: 'help-articles',
        name: 'AdminHelpArticles',
        component: () => import('@/views/system/HelpArticlesPage.vue'),
        meta: { title: '帮助文章', perm: 'support:view' },
      },
      {
        path: 'changelogs',
        name: 'AdminChangelogs',
        component: () => import('@/views/operation/ChangelogsPage.vue'),
        meta: { title: '更新日志', perm: 'operation:view' },
      },
      {
        path: 'agreements',
        name: 'AdminAgreements',
        component: () => import('@/views/operation/AgreementsPage.vue'),
        meta: { title: '用户协议', perm: 'system:view' },
      },
      {
        path: 'realtime',
        name: 'AdminRealtimeMonitor',
        component: () => import('@/views/monitor/RealtimeMonitorPage.vue'),
        meta: { title: '实时监控面板', perm: 'monitor:view' },
      },
      {
        path: 'monitor',
        name: 'AdminMonitor',
        component: () => import('@/views/monitor/MonitorPage.vue'),
        meta: { title: '系统监控', perm: 'monitor:view' },
      },
      {
        path: 'traffic-flow',
        name: 'AdminTrafficFlow',
        component: () => import('@/views/monitor/TrafficFlowPage.vue'),
        meta: { title: '流量流向', perm: 'monitor:view' },
      },
      {
        path: 'model-performance',
        name: 'AdminModelPerformance',
        component: () => import('@/views/monitor/ModelPerformancePage.vue'),
        meta: { title: '模型性能', perm: 'monitor:view' },
      },
      {
        path: 'alert-rules',
        name: 'AdminAlertRules',
        component: () => import('@/views/monitor/AlertRulesPage.vue'),
        meta: { title: '告警规则', perm: 'monitor:view' },
      },
      {
        path: 'alert-events',
        name: 'AdminAlertEvents',
        component: () => import('@/views/monitor/AlertEventsPage.vue'),
        meta: { title: '告警记录', perm: 'monitor:view' },
      },
      {
        path: 'error-logs',
        name: 'AdminErrorLogs',
        component: () => import('@/views/monitor/ErrorLogsPage.vue'),
        meta: { title: '系统错误日志', perm: 'monitor:view' },
      },
      {
        path: 'channel-errors',
        name: 'AdminChannelErrors',
        component: () => import('@/views/monitor/ChannelErrorsPage.vue'),
        meta: { title: '渠道错误监控', perm: 'monitor:view' },
      },
      {
        path: 'content-filter-logs',
        name: 'AdminContentFilterLogs',
        component: () => import('@/views/monitor/ContentFilterLogsPage.vue'),
        meta: { title: '拦截日志', perm: 'audit:view' },
      },
      {
        path: 'cron-jobs',
        name: 'AdminCronJobs',
        component: () => import('@/views/monitor/CronJobsPage.vue'),
        meta: { title: '定时任务', perm: 'system:view' },
      },
    ],
  },
]

export default adminRoutes
