...
// 认证彻底失效时由 auth store 注册的清理回调（清空响应式状态与持久化登录数据）
...
function getLoginRedirectPath(): string {
  ...
  if (!clean || clean.startsWith('/admin/login')) return '/admin'
  return clean
}
...
function handleAuthExpired(): void {
  ...
  // 由 auth store 清空响应式状态与持久化数据（store 自身负责全部清理）
  authExpiredCallback?.()
  // 兜底清除 token storage
  clearTokens()
  Message.warning('登录状态已失效，请重新登录')
  window.location.hash = `#/admin/login?redirect=${encodeURIComponent(getLoginRedirectPath())}`
}
