...
type TskModelTasks struct {
	...
	PreDeductAmount decimal.Decimal `json:"pre_deduct_amount" orm:"pre_deduct_amount" description:"提交任务时的预扣金额（USD），任务完成后根据实际用量结算差额"`                                        // 提交任务时的预扣金额（USD），任务完成后根据实际用量结算差额
	ActualCost      decimal.Decimal `json:"actual_cost"       orm:"actual_cost"       description:"任务实际消费金额（USD），成功时按此结算，失败时退还预扣金额"`                                        // 任务实际消费金额（USD），成功时按此结算，失败时退还预扣金额
