...
type BilWallets struct {
	...
	TotalConsumed      decimal.Decimal  `json:"total_consumed"       orm:"total_consumed"       description:"累计消费总额（本位币；Redis 钱包 hash total_consumed 的物化副本，随结算事件递增/补偿递减）"` // 累计消费总额（本位币；Redis 钱包 hash total_consumed 的物化副本，随结算事件递增/补偿递减）
}
