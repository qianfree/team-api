...
type BilRecords struct {
	...
	InputPrice              decimal.Decimal `json:"input_price"               orm:"input_price"               description:"计费时输入单价（快照，防止价格变更影响历史记录）"`                                  // 计费时输入单价（快照，防止价格变更影响历史记录）
	OutputPrice             decimal.Decimal `json:"output_price"              orm:"output_price"              description:"计费时输出单价（快照）"`                                               // 计费时输出单价（快照）
	TotalCost               decimal.Decimal `json:"total_cost"                orm:"total_cost"                description:"最终费用 = 基础价格 × 模型倍率 × 租户倍率"`                                 // 最终费用 = 基础价格 × 模型倍率 × 租户倍率
	...
	EffectiveInputPrice     decimal.Decimal `json:"effective_input_price"     orm:"effective_input_price"     description:"实际生效的输入单价（快照）"`                                             // 实际生效的输入单价（快照）
	EffectiveOutputPrice    decimal.Decimal `json:"effective_output_price"    orm:"effective_output_price"    description:"实际生效的输出单价（快照）"`                                             // 实际生效的输出单价（快照）
	DiscountRatio           decimal.Decimal `json:"discount_ratio"            orm:"discount_ratio"            description:"实际折扣比例（快照）"`                                                // 实际折扣比例（快照）
	BillingInputMultiplier  decimal.Decimal `json:"billing_input_multiplier"  orm:"billing_input_multiplier"  description:"梯度定价输入乘数（快照）"`                                              // 梯度定价输入乘数（快照）
	BillingOutputMultiplier decimal.Decimal `json:"billing_output_multiplier" orm:"billing_output_multiplier" description:"梯度定价输出乘数（快照）"`                                              // 梯度定价输出乘数（快照）
	...
	ModelMultiplier         decimal.Decimal `json:"model_multiplier"          orm:"model_multiplier"          description:"模型倍率（快照）"`                                                  // 模型倍率（快照）
	TenantMultiplier        decimal.Decimal `json:"tenant_multiplier"         orm:"tenant_multiplier"         description:"租户倍率（快照）"`                                                  // 租户倍率（快照）
	BaseInputPrice          decimal.Decimal `json:"base_input_price"          orm:"base_input_price"          description:"基础模型输入单价（快照，应用倍率前）"`                                        // 基础模型输入单价（快照，应用倍率前）
	BaseOutputPrice         decimal.Decimal `json:"base_output_price"         orm:"base_output_price"         description:"基础模型输出单价（快照，应用倍率前）"`                                        // 基础模型输出单价（快照，应用倍率前）
