...
type TntUsers struct {
	...
	QuotaLimit     decimal.Decimal `json:"quota_limit"     orm:"quota_limit"     description:"额度上限（USD），quota_type 为 none 时忽略"`           // 额度上限（USD），quota_type 为 none 时忽略
	QuotaUsed      decimal.Decimal `json:"quota_used"      orm:"quota_used"      description:"已使用额度（USD）"`                                // 已使用额度（USD）
