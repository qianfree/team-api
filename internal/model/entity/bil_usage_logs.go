...
type BilUsageLogs struct {
	...
	InputTokens           int             `json:"input_tokens"             orm:"input_tokens"             description:"输入 token 数（含缓存总输入：base + cache_read + cache_creation，跨渠道统一口径）"`                           // 输入 token 数（含缓存总输入：base + cache_read + cache_creation，跨渠道统一口径）
	...
	CacheCreationTokens   int             `json:"cache_creation_tokens"    orm:"cache_creation_tokens"    description:"写入缓存的 token 数（Claude cache_creation_input_tokens / OpenAI Responses cache_write_tokens）"` // 写入缓存的 token 数（Claude cache_creation_input_tokens / OpenAI Responses cache_write_tokens）
	...
	UpstreamRequestId     string          `json:"upstream_request_id"      orm:"upstream_request_id"      description:"上游请求ID：对话=上游响应头请求ID（X-Oneapi-Request-Id等），任务=上游任务ID；排障反查用"`                               // 上游请求ID：对话=上游响应头请求ID（X-Oneapi-Request-Id等），任务=上游任务ID；排障反查用
}
