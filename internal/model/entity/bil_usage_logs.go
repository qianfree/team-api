...
type BilUsageLogs struct {
	...
	TotalCost             float64     `json:"total_cost"               orm:"total_cost"               description:"本次调用费用"`                                                    // 本次调用费用
	...
	InputCost             float64     `json:"input_cost"               orm:"input_cost"               description:"输入 token 费用"`                                               // 输入 token 费用
	OutputCost            float64     `json:"output_cost"              orm:"output_cost"              description:"输出 token 费用"`                                               // 输出 token 费用
	...
	ActualCost            float64     `json:"actual_cost"              orm:"actual_cost"              description:"实际扣除费用（含折扣后）"`                                              // 实际扣除费用（含折扣后）
	...
	RequestType           int         `json:"request_type"             orm:"request_type"             description:"请求类型: 1=sync, 2=stream, 3=websocket"`                       // 请求类型: 1=sync, 2=stream, 3=websocket
	...
	RateMultiplier        float64     `json:"rate_multiplier"          orm:"rate_multiplier"          description:"费率倍率/折扣"`                                                   // 费率倍率/折扣
	PreDeductAmount       float64     `json:"pre_deduct_amount"        orm:"pre_deduct_amount"        description:"预扣金额"`                                                      // 预扣金额
	RefundAmount          float64     `json:"refund_amount"            orm:"refund_amount"            description:"退款金额"`                                                      // 退款金额
	SupplementAmount      float64     `json:"supplement_amount"        orm:"supplement_amount"        description:"补扣金额"`                                                      // 补扣金额
	...
	AccountCost           float64     `json:"account_cost"             orm:"account_cost"             description:"上游账户成本（用于利润分析）"`                                            // 上游账户成本（用于利润分析）
