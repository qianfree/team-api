...
type BilTransactions struct {
	...
	Type         string          `json:"type"          orm:"type"          description:"类型：consume（消费）/ recharge（充值入账）/ redemption（兑换码入账）/ refund（退款扣回）/ adjust（调整）/ pre_deduct（预扣，已废弃）/ settle（结算，已废弃）/ freeze（冻结，已废弃）/ unfreeze（解冻，已废弃）"` // 类型：consume（消费）/ recharge（充值入账）/ redemption（兑换码入账）/ refund（退款扣回）/ adjust（调整）/ pre_deduct（预扣，已废弃）/ settle（结算，已废弃）/ freeze（冻结，已废弃）/ unfreeze（解冻，已废弃）
	Amount       decimal.Decimal `json:"amount"        orm:"amount"        description:"变动金额（正数=收入，负数=支出）"`                                                                                                                                 // 变动金额（正数=收入，负数=支出）
	BalanceAfter decimal.Decimal `json:"balance_after" orm:"balance_after" description:"变动后总余额"`                                                                                                                                            // 变动后总余额
	FrozenAfter  decimal.Decimal `json:"frozen_after"  orm:"frozen_after"  description:"变动后冻结余额"`                                                                                                                                           // 变动后冻结余额
