...
// TestRunHandlerSafely_Panic转为错误 robfig/cron v3 的 cron.New() 不带 Recover 链，
// startJob 是裸的 `go j.Run()`——任务 panic 会崩掉整个进程。此处必须兜住并转成
// 普通 error，才能既保住进程、又让失败落进 sys_cron_jobs 供管理后台查看。
func TestRunHandlerSafely_Panic转为错误(t *testing.T) {
	job := &CronJob{
		Name: "panicking_job",
...
func TestRunHandlerSafely_正常返回值透传(t *testing.T) {
...
// TestReschedule_未启动时只更新记录 StartBackground 之前没有 cron 条目，
// 此时重排只改 job.Schedule，供随后的 StartBackground 按新表达式注册。
func TestReschedule_未启动时只更新记录(t *testing.T) {
	cs := NewCronScheduler()
...
func TestReschedule_未注册任务报错(t *testing.T) {
	cs := NewCronScheduler()
	assert.Error(t, cs.Reschedule(context.Background(), "nonexistent", "@every 5m"))
}

// TestReschedule_非法表达式保持原调度 一次错误配置不能把任务从调度器里彻底摘掉，
// 因此实现是「先加新条目、成功后再摘旧条目」。
func TestReschedule_非法表达式保持原调度(t *testing.T) {
	ctx := context.Background()
	cs := NewCronScheduler()
...
// TestReschedule_有效表达式替换条目 同时验证 @every 语法确实被 cron.New() 的
// 默认解析器接受——探测间隔配置正是靠它支持任意分钟数（*/N 在 N>=60 时会静默失真）。
func TestReschedule_有效表达式替换条目(t *testing.T) {
	ctx := context.Background()
	cs := NewCronScheduler()
...
// TestOnSettingsChanged_回调panic不影响其余回调 回调在订阅 goroutine 内串行执行，
// 一个回调 panic 不能带崩订阅循环或阻断后续回调。
func TestOnSettingsChanged_回调panic不影响其余回调(t *testing.T) {
	settingsHookMu.Lock()
	saved := settingsHooks
	settingsHooks = nil
