...
var Registry = []SettingDef{
	...
	{Key: "channel_auto_test_interval_minutes", Type: SettingTypeInt, Default: "5", Category: "channel",
		Label: "自动探测间隔（分钟）", Validation: "min:1,max:1440",
		Description: "两轮自动探测之间的间隔，保存后即时生效。探测是串行的（每渠道最多 30 秒），渠道多或上游超时多时单轮耗时会变长；上一轮未结束时下一轮自动跳过，因此间隔不宜小于「渠道数 × 30 秒」"},
	...
	{Key: "sync_image_async_enabled", Type: SettingTypeBool, Default: "false", Category: "channel",
		Label: "同步图片厂商异步化", Description: "开启后，同步阻塞返回的图片厂商（如 OpenAI/DALL·E）走 /v1/images/generations/async 时由后台 worker 池异步处理，客户端提交即拿 task_id 后轮询取图；关闭则该端点对同步厂商返回不支持。依赖对象存储保存生成结果，开启前需先在「存储配置」中配置对象存储（OSS/S3/COS）"},
	...
	{Key: "relay_claude_thinking_adapter_enabled", Type: SettingTypeBool, Default: "true", Category: "channel",
		Label: "Claude thinking 后缀适配", Description: "开启后，OpenAI 入站 × Claude 上游把模型名 -thinking/effort 后缀转换为扩展思考请求（注入 thinking 配置）"},
	{Key: "relay_claude_thinking_budget_percentage", Type: SettingTypeFloat, Default: "0.5", Category: "channel",
		Label: "Claude 思考预算比例", Description: "thinking 适配触发时按 max_tokens 的该比例确定 budget_tokens（下限 1024）", Validation: "min:0.1,max:0.9"},
	{Key: "relay_gemini_thinking_adapter_enabled", Type: SettingTypeBool, Default: "true", Category: "channel",
		Label: "Gemini thinking 后缀适配", Description: "开启后，OpenAI 入站 × Gemini 上游把模型名 -thinking/-nothinking/effort 后缀映射到 thinkingConfig"},
	{Key: "relay_gemini_thinking_budget_percentage", Type: SettingTypeFloat, Default: "0.5", Category: "channel",
		Label: "Gemini 思考预算比例", Description: "thinking 适配触发时按 maxOutputTokens 的该比例确定 thoughtBudget（下限 128）", Validation: "min:0.1,max:0.9"},
	{Key: "relay_gemini_thought_signature_enabled", Type: SettingTypeBool, Default: "true", Category: "channel",
		Label: "Gemini thoughtSignature 透传", Description: "开启后，Gemini 上游的 function-call parts 附带 thoughtSignature 绕过值（多轮工具调用校验所需）"},
	{Key: "relay_gemini_safety_setting", Type: SettingTypeJSON, Default: "", Category: "channel",
		Label: "Gemini 安全阈值（JSON）", Description: "类别 → 伤害阈值映射，如 {\"HARM_CATEGORY_HARASSMENT\":\"BLOCK_ONLY_HIGH\"}；转换后的 Gemini 请求按此附带 safetySettings；留空不附带"},
	{Key: "relay_preserve_thinking_suffix_models", Type: SettingTypeString, Default: "", Category: "channel",
		Label: "保留 thinking 后缀的模型", Description: "逗号分隔的模型名列表，列表内模型（支持尾部 * 前缀匹配）在发往上游的模型名上保留 -thinking/-nothinking/effort 后缀；留空对所有模型剥离后缀"},
