...
//
// Security: Only business errors (code >= 10000), validation errors, and standard HTTP 4xx errors
...
func Error(r *ghttp.Request, err error) {
	...
	// Check for GoFrame validation error first (gvalid.Error is NOT a gerror.Error,
	// so the gerror branch below won't match it — must check separately)
	var validErr gvalid.Error
	if errors.As(err, &validErr) {
		code = 422
		msg = validErr.Current().Error()
	} else {
