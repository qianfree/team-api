...
//
// 编号策略说明（解释段内出现的"空洞"与跳跃，避免误判为漏号）：
//   - HTTP 语义码（400/401/403/404/429/500）直接复用，不归入业务段。
//   - 业务码从 10000 起，按功能域分段（每段预留约 100 号），新增域从下一个
//     百位段起分配（如 10400+），方便聚合统计与前端按域归类。
//   - 段内编号按"先来先得"分配，删除/废弃的码不回收复用（避免历史日志里的旧码
//     被赋予新含义造成歧义），因此段内会出现空洞（如 10067/10070-72/10099）。
//   - 跨段跳跃（如 10097→10403）源于不同功能域在不同迭代落地，分段时间线不同。
//   - Demo/Plugin/Update/Team 等较新域集中在 10400+，与早期 10xxx 域拉开距离，
//     便于后续按前缀做特性灰度与告警分组。
//
// 新增错误码时：先归属功能域，在该域已用最大值后递增分配；新域从下一个空闲百位段起。
const (
...
const (
	...
	MsgModelUnavailable    = "当前模型暂时不可用"
	MsgRequestTimeout      = "请求超时，请重试"
	MsgRequestFailed       = "请求处理失败"
