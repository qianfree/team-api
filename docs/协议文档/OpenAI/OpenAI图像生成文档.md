...
| 端点 | 方法 | 路径 | 功能 | 支持模型 |
|------|------|------|------|----------|
| 图像生成 | POST | `/v1/images/generations` | 根据文本提示生成图像 | `dall-e-2`, `dall-e-3`, `gpt-image-1`, `gpt-image-1-mini`, `gpt-image-1.5`, `gpt-image-2` |
| 图像编辑 | POST | `/v1/images/edits` | 编辑现有图像（支持多图） | `dall-e-2`, `gpt-image-1`, `gpt-image-2` |
...
| `model` | string | **是** | `dall-e-2` | — | 模型 ID：`dall-e-2`、`dall-e-3`、`gpt-image-1`、`gpt-image-1-mini`、`gpt-image-1.5`、`gpt-image-2` |
...
| `background` | string | 否 | `auto` | GPT Image | 背景透明度：`transparent`、`opaque`、`auto`。仅支持 PNG/WebP 输出格式。**gpt-image-2 不支持 `transparent`** |
...
仅 GPT Image 模型（`gpt-image-1`、`gpt-image-1-mini`、`gpt-image-1.5`、`gpt-image-2`）支持流式响应。通过设置 `stream: true` 或 `partial_images > 0` 启用。
...
| 参数 | dall-e-2 | dall-e-3 | gpt-image-1 / mini / 1.5 | gpt-image-2 |
|------|----------|----------|--------------------------|-------------|
| **生成端点** | 支持 | 支持 | 支持 | 支持 |
| **编辑端点** | 支持（mask） | 不支持 | 支持（多图，无 mask） | 支持（多图，无 mask） |
| **变体端点** | 支持 | 不支持 | 不支持 | 不支持 |
| **prompt 最大长度** | 1000 字符 | 4000 字符 | 32000 字符 | 32000 字符 |
| **n（生成数量）** | 1 | 1 | 1-10 | 1-10 |
| **尺寸** | 256/512/1024 正方形 | 1024 正方形 + 横纵 1792 | 1024 正方形 + 1536 横纵 | 灵活分辨率（最大边 ≤ 3840px，16 倍数，比例 ≤ 3:1） |
| **quality** | 不支持 | `standard`/`hd` | `auto`/`high`/`medium`/`low` | `auto`/`high`/`medium`/`low` |
| **style** | 不支持 | `vivid`/`natural` | 不支持 | 不支持 |
| **background** | 不支持 | 不支持 | `transparent`/`opaque`/`auto` | `opaque`/`auto`（**不支持 transparent**） |
| **output_format** | 不支持 | 不支持 | `png`/`jpeg`/`webp` | `png`/`jpeg`/`webp` |
| **output_compression** | 不支持 | 不支持 | 0-100 | 0-100 |
| **流式响应** | 不支持 | 不支持 | 支持（`partial_images`） | 支持（`partial_images`） |
| **moderation** | 不支持 | 不支持 | `auto`/`low` | `auto`/`low` |
| **response_format 默认值** | `url` | `url` | `b64_json` | `b64_json` |
| **Usage 统计** | 不返回 | 不返回 | 返回 | 返回 |
...
1. ~~**GPT Image 新参数未覆盖**~~：已覆盖（`ImageRequest` 已包含 `partial_images`、`stream` 等字段）
2. ~~**GPT Image 增强响应未覆盖**~~：已覆盖（`ImageResponse` 已包含 `background`、`output_format`、`size`、`quality`、`usage` 字段）
3. **图像编辑端点未注册**：`/v1/images/edits` 路径尚未在 `Path2RelayMode` 中注册
4. **图像变体端点未注册**：`/v1/images/variations` 路径尚未在 `Path2RelayMode` 中注册
5. ~~**流式图像响应未实现**~~：已实现（`handleImageStreamResponse` 支持 SSE 流式）
6. ~~**Usage 统计缺失**~~：已实现（`handleImageResponse` 从 GPT Image 响应中提取 `usage`）
7. **计费缺失**：DALL-E 和 GPT Image 的定价模型不同（DALL-E 按次计费，GPT Image 按 token 计费），需要区分

### 11.4 DTO 扩展（已实现）

以下 DTO 扩展已在 `relay/dto/usage.go` 中实现：
...
| `gpt-image-2` | 按 token 计费 | text input $5/M, image input $8/M, text output $10/M, image output $30/M tokens |

Relay 层的 `BillingProvider` 需要根据模型类型选择不同的计费策略：DALL-E 模型使用固定价格计费，GPT Image 模型使用 token 计费。`handleImageResponse` 已支持从 GPT Image 响应中提取 `usage` 字段用于计费。
