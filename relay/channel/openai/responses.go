...
// 转换由 relaykit 转换器完成（openai → openai_responses 方向），本函数只保留非 200
// 错误透传与桥接调用（hard-fail）；计费用量按上游原始 chat 体口径提取（prompt 已含缓存）。
func (a *Adaptor) handleResponsesInboundNonStream(ctx context.Context, resp *http.Response, info *common.RelayInfo, writer http.ResponseWriter) (*common.Usage, error) {
	...
	// relaykit 响应转换（唯一路径，hard-fail：解析/转换失败即向上返回错误）
	convertedBody, _, handled, convErr := relaykit_bridge.TryConvertResponseViaRelaykit(ctx, info, body)
	if !handled {
		return nil, constant.NewChannelError("openai adaptor: no relaykit converter for responses client response", nil)
	}
	if convErr != nil {
...
// 转换由 relaykit 流式转换器完成（openai → openai_responses 方向），本函数只保留
// 非 200 错误透传与桥接调用（hard-fail：桥接层未接管即注册缺失，显式报错）。
func (a *Adaptor) handleResponsesInboundStream(ctx context.Context, resp *http.Response, info *common.RelayInfo, writer http.ResponseWriter) (*common.Usage, error) {
	...
	if usage, ok, err := relaykit_bridge.TryConvertStreamViaRelaykit(ctx, info, resp.Body, writer); ok {
		// 桥接层已写出 SSE 并完成收尾裁决与 usage 捕获，err 非空即流以错误/中断结束
		return usage, err
	}
	return nil, constant.NewChannelError("openai adaptor: relaykit stream converter unavailable for responses client", nil)
}
