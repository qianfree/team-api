...
// 供应商在格式转换之外还有私有请求适配——模型名后缀剥离（xai 的 -search/-high、
// baidu_v2 的 -search）、私有开关注入（xai search_parameters、baidu_v2 web_search）、
// 参数兼容（zhipu GLM top_p 裁剪、ali DashScope thinking_budget 剥离）、
// 供应商方言的思考参数（deepseek/zhipu 的 thinking 对象）。这些都不属于协议转换，
...
//   - 必须幂等于 relaykit 已做的通用处理（模型名替换、stream_options、reasoning_effort）：
